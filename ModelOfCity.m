...
x1=zeros(1,19); x2=zeros(1,15);
Age2OwnHurt=0; Age14OwnHurt=0; AvgBenefit=0; AvgOwnBenefit=0; AvgRentBenefit=0;

% The version number here refers to the sequence of shocks. 
% Usually, we only run the code once and set the version number to 0.
% For the cases in Table 2, we run the fortran code, "oot.f", for 5 times with 
...
tax_neutral=(taxpropTimeSeries(1,1)*sum((H1.*PH1+H2.*PH2).*in1)/sum(in1)-(taxpropTimeSeries(1,1)+...
    taxpropOOT(1))*sum((ForDemTot(:,1).*PH1+ForDemTot(:,2).*PH2).*in2)/sum(in2))/...
    (sum(((H1-ForDemTot(:,1)).*PH1+(H2-ForDemTot(:,2)).*PH2).*in2)/sum(in2));
...
inw1=inw==1 & LOCind==1;
inw2=inw==1 & LOCind==2;
inlowprod=(iZind==1 | iZind==4);
inlowprodr=(iZind==1 | iZind==4) & RenterInd==1;
inlowprodo=(iZind==1 | iZind==4) & RenterInd==2;
inmidprod=(iZind==2 | iZind==5);
inmidprodr=(iZind==2 | iZind==5) & RenterInd==1;
inmidprodo=(iZind==2 | iZind==5) & RenterInd==2;
inhighprod=(iZind==3 | iZind==6);
inhighprodr=(iZind==3 | iZind==6) & RenterInd==1;
inhighprodo=(iZind==3 | iZind==6) & RenterInd==2;
...
Wt1=inw1./sum(inw1,2);
Wt2=inw2./sum(inw2,2);
Wtlowprod=inlowprod./sum(inlowprod,2);
Wtlowprodr=inlowprodr./sum(inlowprodr,2);
Wtlowprodo=inlowprodo./sum(inlowprodo,2);
Wtmidprod=inmidprod./sum(inmidprod,2);
Wtmidprodr=inmidprodr./sum(inmidprodr,2);
Wtmidprodo=inmidprodo./sum(inmidprodo,2);
Wthighprod=inhighprod./sum(inhighprod,2);
Wthighprodr=inhighprodr./sum(inhighprodr,2);
Wthighprodo=inhighprodo./sum(inhighprodo,2);
...
AvgHoursLowProd=sum(Wtlowprod.*HoursInd,2);
AvgHoursLowProdr=sum(Wtlowprodr.*HoursInd,2);
AvgHoursLowProdo=sum(Wtlowprodo.*HoursInd,2);
AvgHoursMidProd=sum(Wtmidprod.*HoursInd,2);
AvgHoursMidProdr=sum(Wtmidprodr.*HoursInd,2);
AvgHoursMidProdo=sum(Wtmidprodo.*HoursInd,2);
AvgHoursHighProd=sum(Wthighprod.*HoursInd,2);
AvgHoursHighProdr=sum(Wthighprodr.*HoursInd,2);
AvgHoursHighProdo=sum(Wthighprodo.*HoursInd,2);
...
for t=1:T
    inL1=LOCind(t,:)==1;
    inL2=LOCind(t,:)==2; 
    zoneNum(t,1:2)=[sum(inL1) sum(inL2)];
    ...
    for i=1:nAge
        ...
        zoneNum(t,2+(i-1)*2+1)=sum(in);
        zoneOwners(t,2+(i-1)*2+1)=sum(RenterInd(t,in)==2);
        in=LOCind(t,:)==2 & AgeInd(t,:)==i; 
        zoneOwners(t,2+(i-1)*2+2)=sum(RenterInd(t,in)==2);
        zoneNum(t,2+(i-1)*2+2)=sum(in);
    end
end
...
AvgHoursLowProdnext=[AvgHoursLowProd(2:end,:); AvgHoursLowProd(end,:)];
AvgHoursLowProdrnext=[AvgHoursLowProdr(2:end,:); AvgHoursLowProdr(end,:)];
AvgHoursLowProdonext=[AvgHoursLowProdo(2:end,:); AvgHoursLowProdo(end,:)];
AvgHoursMidProdnext=[AvgHoursMidProd(2:end,:); AvgHoursMidProd(end,:)];
AvgHoursMidProdrnext=[AvgHoursMidProdr(2:end,:); AvgHoursMidProdr(end,:)];
AvgHoursMidProdonext=[AvgHoursMidProdo(2:end,:); AvgHoursMidProdo(end,:)];
AvgHoursHighProdnext=[AvgHoursHighProd(2:end,:); AvgHoursHighProd(end,:)];
AvgHoursHighProdrnext=[AvgHoursHighProdr(2:end,:); AvgHoursHighProdr(end,:)];
AvgHoursHighProdonext=[AvgHoursHighProdo(2:end,:); AvgHoursHighProdo(end,:)];

x1=(x1*version+[mean([outnext(in,[2 3 4 10 11])./out(in,[2 3 4 10 11]) ... %Wage Rent1 Rent2 Price1 Price2 
   ...
   ZoneTwoFracnext(in)./ZoneTwoFrac(in)])-1 ...% Fraction of Population in Zone 1
   Tbl_dW(21,[1 3 5]) ]*100)/(version+1);

INV=(outnext(:,7)-(1-deprH(1))*out(:,7))+(outnext(:,8)-(1-deprH(1))*out(:,8))-(out(:,7)-(1-deprH(1))*out(:,7))+(out(:,8)-(1-deprH(1))*out(:,8));
INVnext=[INV(2:end,:); INV(end,:)];

mean((INVnext(in)-INV(in))./sum(out(in,7:8),2)./sum(ForDemTot(reverse_in,:),2))

mean((INVnext(in)-INV(in))./sum(ForDemTot(reverse_in,:),2))

(mean(INVnext(in2))-mean(INV(in1)))/mean(sum(ForDemTot(in2,:),2))
...
rentincreasesh=(mean(sum(outnext(in,[3 4]).*out(in,7),2)./sum(out(in,[3 4]).*out(in,7),2))-1)*100
priceincreasesh=(mean(sum(outnext(in,[10 11]).*out(in,7),2)./sum(out(in,[10 11]).*out(in,7),2))-1)*100


HoursLowProdCh=(mean([AvgHoursLowProdnext(in)./AvgHoursLowProd(in)...
    AvgHoursLowProdrnext(in)./AvgHoursLowProdr(in)...
    AvgHoursLowProdonext(in)./AvgHoursLowProdo(in)])-1)*100;
HoursMidProdCh=(mean([AvgHoursMidProdnext(in)./AvgHoursMidProd(in)...
    AvgHoursMidProdrnext(in)./AvgHoursMidProdr(in)...
    AvgHoursMidProdonext(in)./AvgHoursMidProdo(in)])-1)*100;
HoursHighProdCh=(mean([AvgHoursHighProdnext(in)./AvgHoursHighProd(in)...
    AvgHoursHighProdrnext(in)./AvgHoursHighProdr(in)...
    AvgHoursHighProdonext(in)./AvgHoursHighProdo(in)])-1)*100;
%%
...
% if mode==1 && (model==1 || model==length(model_space)-1 || model==length(model_space)-2 || model==length(model_space)-3) && plot==1
%     if version==0
...
if mode==1 && (model==1 || model==length(model_space)-1 || model==length(model_space)-2 || model==length(model_space)-3)
    if version==0
        hprod=[HoursLowProdCh; HoursMidProdCh; HoursHighProdCh];
    else
        hprod=(hprod*version+[HoursLowProdCh; HoursMidProdCh; HoursHighProdCh])/(version+1);
    end
    x=categorical(["Low","Med","High"]);  
    x=reordercats(x,{'Low','Med','High'});
    path_fig="../Figures/";
    openfig(join([path_fig, "labor_change_by_productivity"], ""))
    if model==1
    subplot(2,2,1);
    bar(x,hprod,'grouped');
%     ylim([-4,1])
%     yticks(-4:1:1)
    ylabel('Labor Supply Change (%)')
    title({'(a) OOT tax rate = 6.3% + 0%', 'all to public goods'});
    legend('all', 'renter', 'owner', 'Position',[0.35 0.83 0.02 0.02]);
    savefig(join([path_fig, "labor_change_by_productivity"], ""))
    saveas(gcf,join([path_fig, "labor_change_by_productivity.png"], ""))
    elseif model==length(model_space)-3
    subplot(2,2,2); 
    bar(x,hprod,'grouped');
%     ylim([-4,1])
...
rentincreaselo=(mean(sum(out(in2,[3 4]).*out(in2,7),2))/mean(sum(out(in1,[3 4]).*out(in1,7),2))-1)*100
priceincreaselo=(mean(sum(out(in2,[10 11]).*out(in2,7),2))/mean(sum(out(in1,[10 11]).*out(in1,7),2))-1)*100
...
%          fprintf(fID,join(['\\newcommand{\\HoursMinBind}[2]{\\ifnum #2=1 \\num[round-mode=places,round-precision=#1]{%4.4f} \\else \\num[round-mode=places,round-precision=#1]','{%4.4f}\\fi}\n']),[abs(HoursMinBind*100),HoursMinBind*100])  ;   
%          fprintf(fID,join(['\\newcommand{\\taxssper}[2]{\\ifnum #2=1 \\num[round-mode=places,round-precision=#1]{%4.4f} \\else \\num[round-mode=places,round-precision=#1]','{%4.4f}\\fi}\n']),[abs(fracRet*100),fracRet*100])  ;   
%          fprintf(fID,join(['\\newcommand{\\Ppsizone}[2]{\\ifnum #2=1 \\num[round-mode=places,round-precision=#1]{%4.4f} \\else \\num[round-mode=places,round-precision=#1]','{%4.4f}\\fi}\n']),[abs(fracRet*100),fracRet*100])  ;   
%          fprintf(fID,join(['\\newcommand{\\Ppsiztwo}[2]{\\ifnum #2=1 \\num[round-mode=places,round-precision=#1]{%4.4f} \\else \\num[round-mode=places,round-precision=#1]','{%4.4f}\\fi}\n']),[abs(fracRet*100),fracRet*100])  ;   
%          fprintf(fID,join(['\\newcommand{\\Ppsizthree}[2]{\\ifnum #2=1 \\num[round-mode=places,round-precision=#1]{%4.4f} \\else \\num[round-mode=places,round-precision=#1]','{%4.4f}\\fi}\n']),[abs(fracRet*100),fracRet*100])  ;   
%          fprintf(fID,join(['\\newcommand{\\RetIncbyLabInc}[2]{\\ifnum #2=1 \\num[round-mode=places,round-precision=#1]{%4.4f} \\else \\num[round-mode=places,round-precision=#1]','{%4.4f}\\fi}\n']),[abs(fracRet*100),fracRet*100])  ;   
...
%         fprintf(fID,join(['\\newcommand{\\',num2str(model_space(model)),'SSZoneOneFracP}','{%4.1f}\n']),x2(15))  ;     
        ...
        %Welfare
        fprintf(fID,join(['\\newcommand{\\',num2str(model_space(model)),'AgeTwoOwnHurt}','{%4.1f}\n']),Age2OwnHurt*100)  ;     
        fprintf(fID,join(['\\newcommand{\\',num2str(model_space(model)),'AgeForteenOwnHurt}','{%4.1f}\n']),Age14OwnHurt*100)  ;     
